...
export type PersonaSlice = { name: string; tone: string | null; instructions: string };
 ...
 */
export function buildAdaptiveScenarioPlan(args: {
  ...
  const coherence = mergeWeaknessesForScenario(targetWeaknesses);
  ...
  const realism = realismSeed ? deriveSimulationRealism(realismSeed, persona.name) : null;
  ...
  for (const skill of skills) {
    if (realismSeed) {
      const idx = pickPressureIndexFromSeed(realismSeed, skill, `primary:${stageSalt}:${knowledgeSalt}`);
      tactics.push(pickPressureByIndex(skill, idx));
    } else {
      tactics.push(pickPressureByIndex(skill, 0));
    }
    // With multiple focus areas, one tactic per skill avoids stacking too many competing pressures.
    if (skills.length === 1) {
      if (realismSeed) {
        const idx2 = pickPressureIndexFromSeed(
          realismSeed,
          skill,
          `secondary:${stageSalt}:${knowledgeSalt}`
        );
        tactics.push(pickPressureByIndex(skill, idx2));
      } else {
        tactics.push(pickPressureByIndex(skill, 1));
      }
    }
  }
  ...
  const base: AdaptiveScenarioPlan = {
  ...
  return realismSeed ? enrichAdaptiveScenarioPlanWithRealism(base, realismSeed, persona.name) : base;
}
