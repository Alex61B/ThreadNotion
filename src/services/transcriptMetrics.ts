 ...
 */
export function computeTranscriptMetrics(messages: MessageLike[]): {
  ...
  const avgMessageLength =
    userMsgs.length > 0 ? userChars / userMsgs.length : 0;
...
export function formatTranscript(messages: MessageLike[]): string {
 ...
 */
export function formatTranscriptNumbered(messages: MessageLike[]): string {
  return messages
