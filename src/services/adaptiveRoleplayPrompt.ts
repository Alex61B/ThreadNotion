...
function realismBlock(plan: AdaptiveScenarioPlan): string {
  const r = plan.simulationRealism;
  if (!r) return '';

  const knowledge =
    r.buyerKnowledgeLevel === 'uninformed'
  ...
  const behavior =
    r.customerBehavior === 'cooperative'
  ...
  const comms =
    r.personaTraits.communicationStyle === 'concise'
  ...
  const pace =
    r.personaTraits.timePressure === 'high'
