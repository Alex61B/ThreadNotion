 ...
 */
export function hasAnySessionCookieName(cookieNames: readonly string[]): boolean {
  const set = new Set(cookieNames);
  for (const base of SESSION_COOKIE_BASE_NAMES) {
    if (set.has(base)) return true;
    if (cookieNames.some((n) => n === `${base}.0` || n.startsWith(`${base}.`))) return true;
  }
  return false;
}
...
export function hasAuthSessionCookie(hasCookie: (name: string) => boolean): boolean {
  return SESSION_COOKIE_BASE_NAMES.some((base) => {
    if (hasCookie(base)) return true;
    // Chunked session cookies: base.0, base.1, …
    for (let i = 0; i < 12; i++) {
      if (hasCookie(`${base}.${i}`)) return true;
    }
    return false;
  });
}
 ...
 */
export function getSessionTokenFromParsedCookies(cookies: Record<string, string>): string | null {
  for (const base of SESSION_COOKIE_BASE_NAMES) {
