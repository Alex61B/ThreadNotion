...
exports.linkAnonymousToAuth = linkAnonymousToAuth;
function normalizeId(id) {
    return id.trim();
}
function unionFocusSkills(a, b) {
 ...
 */
async function linkAnonymousToAuth(prisma, args) {
    ...
    try {
        txResult = await prisma.$transaction(async (tx) => {
            ...
            if (anonFocus && !authFocus) {
                ...
                const sessions = minSessions(authFocus.sessionsRemaining, anonFocus.sessionsRemaining);
                await tx.userTrainingFocus.update({
                    where: { userId: authUserId },
                    data: {
                        focusSkills: merged,
...
//# sourceMappingURL=anonymousUserLinkService.js.map